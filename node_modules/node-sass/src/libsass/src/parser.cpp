...
// fonts, as in `rem(16px)/rem(24px)`). When parsing lists and expression we
...
namespace Sass {
  ...
  Arguments_Obj Parser::parse_arguments()
  {
    ...
    if (lex_css< exactly<'('> >()) {
      ...
      if (!lex_css< exactly<')'> >()) {
        css_error("Invalid CSS", " after ", ": expected expression (e.g. rem(1px), bold), was ");
      }
    }
    return args;
  }
  ...
  Expression_Obj Parser::parse_value()
  {
    ...
    // parse `10%rem(4px)` into separated items and not a schema
  ...
  While_Obj Parser::parse_while_directive()
  {
    ...
    if (!predicate || (l && !l->length())) {
      css_error("Invalid CSS", " after ", ": expected expression (e.g. rem(1px), bold), was ", false);
    }
